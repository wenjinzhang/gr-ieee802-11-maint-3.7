...
int general_work (int noutput, gr_vector_int& ninput_items,
	...
	if (d_tags.size()) {
		...
		if(offset > nread) {
			...
			d_freq_offset_short = pmt::to_double(pmt::dict_ref(d_tags.front().value, pmt::mp("freq_offset_short"), pmt::from_double(0)));
                        d_sof = pmt::to_uint64(pmt::dict_ref(d_tags.front().value, pmt::mp("sof"), pmt::from_uint64(0)));
			dout << "LONG | updating state to " <<  d_state << ", updating d_sof: " << d_sof << std::endl;
		...
		if(!rel)  {
		    ...
		    pmt::pmt_t dict = pmt::make_dict();
				 ...
				 pmt::string_to_symbol(name()));
                    ...
                    dout << "LONG | adding tag to stream: sof = " << d_sof << std::endl;	
...
private:
	...
	uint64_t    d_sof;  // [X.Z. 2019-03-26] added. 
