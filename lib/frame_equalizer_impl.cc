...
frame_equalizer_impl::frame_equalizer_impl(Equalizer algo, double freq, double bw, bool log, bool debug) :
	...
	message_port_register_out(pmt::mp("symbols field"));
...
frame_equalizer_impl::general_work (int noutput_items,
	...
	// myfile1.open("encoding.txt", std::ios::app);
	...
	while((i < ninput_items[0]) && (o < noutput_items)) {
        ...
        d_sof = pmt::to_uint64(pmt::dict_ref(tags.front().value, pmt::mp("sof"), pmt::from_uint64(0)));        
		d_frame_start = pmt::to_uint64(pmt::dict_ref(tags.front().value, 
							     pmt::mp("frame_start"),
							     pmt::from_uint64(0)));

        double fo = pmt::to_double(pmt::dict_ref(tags.front().value, 
							       pmt::mp("freq_offset_long"), 
							       pmt::from_double(0)));
        
        d_epsilon0 = fo * d_bw / (2 * M_PI * d_freq); 
        d_er = 0;
        
		// dout << "EQUALIZER: d_sof = " << d_sof << std::endl;
		// dout << "EQUALIZER: d_frame_start = " << d_sof << std::endl;
                // dout << "EQUALIZER: epsilon = " << d_epsilon0 << std::endl;
                // dout << "EQUALIZER: d_current_symbol = " << d_current_symbol << std::endl;
                // dout << "EQUALIZER: d_frame_symbols= " << d_frame_symbols << std::endl; 
		// dout << "Parse EQUALIZER " << std::endl;
		...
		dict = pmt::dict_add(dict, pmt::mp("frame_start"), 
					   pmt::from_uint64(d_frame_start));
		dict = pmt::dict_add(dict, pmt::mp("symbol_id"), 
					   pmt::from_uint64(d_current_symbol));

	        // saving pre-equalized frequency-domain preamble in tag
		pmt::pmt_t preamble_in_f = pmt::make_c32vector(64, 0); 
		for (int k=0; k<64; k++)
			pmt::c32vector_set(preamble_in_f, k, current_symbol[k]); 

		// cout << "Write CSI to file " << std::endl;
		// myfile << preamble_in_f << std::endl;
		// dout << preamble_in_f << std::endl;

		dict = pmt::dict_add(dict, pmt::mp("pre_in_f"), preamble_in_f); 
		...
		if(d_current_symbol == 2) {
			...
			// [X.Z. 2019-03-26] start of modification	
                        ...
                        dict = pmt::dict_add(dict, pmt::mp("bits_per_symbol"), 
						   pmt::from_long(d_frame_mod->bits_per_symbol())); 
                        dict = pmt::dict_add(dict, pmt::mp("frame_symbols"), 
					           pmt::from_long(d_frame_symbols)); 
                        dict = pmt::dict_add(dict, pmt::mp("sof"), pmt::from_uint64(d_sof));
                        dict = pmt::dict_add(dict, pmt::mp("frame_start"), 
						   pmt::from_uint64(d_frame_start));
                        dict = pmt::dict_add(dict, pmt::mp("reserved"), 
						   pmt::from_uint64(d_reserved));        
			dict = pmt::dict_add(dict, pmt::mp("rate"), pmt::from_uint64(d_rate));
			// [X.Z. 2019-03-26] end of modification	
			...
			// myfile << preamble_in_f << std::endl;
			// myfile1 << "Rate: "<< pmt::from_uint64(d_rate) << "; Encoding:"<< d_frame_encoding<< std::endl;
		    }
		}
