...
parse_mac_impl(bool log, bool debug) :
	...
	// [X.Z. 2019-03-26] start of modification
	message_port_register_out(pmt::mp("out"));
	// [X.Z. 2019-03-26] end of modification
}
...
void parse(pmt::pmt_t msg) {
	...
	pmt::pmt_t dict = pmt::car(msg); 
        d_sof = pmt::to_uint64(pmt::dict_ref(dict, pmt::mp("sof"), pmt::from_uint64(0)));
        d_snr = pmt::to_double(pmt::dict_ref(dict, pmt::mp("snr"), pmt::from_double(0)));
	
	dout << ">>>>>>>>> ....... >>>>>>>" << std::endl;
        dout << "[XZ] Tagged Info of Mac Frame: d_sof = "<< d_sof<<", d_snr = " << d_snr << std::endl; 
	// [X.Z. 2019-03-26] end of modification
...
void parse_management(char *buf, int length) {
	...
	dout<< "start parsing mac address"<< std::endl;
	}
	...
	// [X.Z. 2019-03-26] start of modification
	if (d_beacon) {
		dout << "[XZ] adding frame info for Beacon: sof = " << d_sof << std::endl; 
	}

	// [X.Z. 2019-06-05] start of modification: add tagging for all management frames
	pmt::pmt_t mac1 = pmt::make_u32vector(6, 0);
	pmt::pmt_t mac2 = pmt::make_u32vector(6, 0);
	pmt::pmt_t mac3 = pmt::make_u32vector(6, 0);
        for (int k=0; k<6; k++)
	{
            pmt::u32vector_set(mac1, k, uint32_t(h->addr1[k]));
            pmt::u32vector_set(mac2, k, uint32_t(h->addr2[k]));
            pmt::u32vector_set(mac3, k, uint32_t(h->addr3[k]));
	}

	// create PDU
        pmt::pmt_t dict = pmt::make_dict();
        dict = pmt::dict_add(dict, pmt::mp("sof"), pmt::from_uint64(d_sof));
        dict = pmt::dict_add(dict, pmt::mp("snr"), pmt::from_double(d_snr));
        dict = pmt::dict_add(dict, pmt::mp("fc"), pmt::from_uint64(h->frame_control));
	dict = pmt::dict_add(dict, pmt::mp("mac1"), mac1);
	dict = pmt::dict_add(dict, pmt::mp("mac2"), mac2);
	dict = pmt::dict_add(dict, pmt::mp("mac3"), mac3);
	message_port_pub(pmt::mp("out"), pmt::cons( pmt::PMT_T, dict));

	// [C.S. 2020-10-27]  
	//myfile << mac1 << std::endl;
	//myfile << mac2 << std::endl;
	//myfile << mac3 << std::endl;
		
	dout << "[XZ] adding mac info to management frame " << std::endl; 
	dout << "<<<<<<<<< ......... <<<<<<<<<<<<" << std::endl;
	// reset the beacon flag
	d_beacon = false; 	
	// [X.Z. 2019-03-26] end of modification
	// [X.Z. 2019-06-05] end of modification: log meta data for all management frames
	// myfile.close();
}
...
void parse_data(char *buf, int length) {
	...
	// [X.Z. 2019-06-05] start of modification: add tagging for all data frames
	pmt::pmt_t mac1 = pmt::make_u32vector(6, 0);
	pmt::pmt_t mac2 = pmt::make_u32vector(6, 0);
	pmt::pmt_t mac3 = pmt::make_u32vector(6, 0);
        for (int k=0; k<6; k++)
	...
	message_port_pub(pmt::mp("out"), pmt::cons( pmt::PMT_T, dict));
        ...
        dout << "MAC Address:s" << mac1 << mac2 << mac1 << std::endl;
	dout << "[XZ] adding mac info to data frame " << std::endl; 
	dout << "<<<<<<<<< ......... <<<<<<<<<<<<" << std::endl;
	// [X.Z. 2019-06-05] end of modification: add tagging for all data frames
}
...
void parse_control(char *buf, int length) {
	...
	// [X.Z. 2019-06-05] start of modification: add tagging for all control frames
	pmt::pmt_t mac1 = pmt::make_u32vector(6, 0);
	pmt::pmt_t mac2 = pmt::make_u32vector(6, 0);
	pmt::pmt_t mac3 = pmt::make_u32vector(6, 0);
        for (int k=0; k<6; k++)
	...
	dout << "[XZ] adding mac info to control frame " << std::endl; 
	dout << "<<<<<<<<< ......... <<<<<<<<<<<<" << std::endl;
	// [X.Z. 2019-06-05] end of modification: add tagging for all data frames
}
...
private:
	...
	// [X.Z. 2019-03-26] start of modification
	bool d_beacon;  // whether the current frame is a Beacon or not
	uint64_t d_sof; // start-of-frame in symbol passed along from "wifi_start" tag stream
	double d_snr; 	// SNR value passed along from "wifi_start" tag stream
	// [X.Z. 2019-03-26] end of modification
};
