...
public:
decode_mac_impl(bool log, bool debug, int channel) :
	block("decode_mac",
...
int general_work (int noutput_items, gr_vector_int& ninput_items,
	...
	ofstream myfile1;
	if(d_channel==0){
		myfile1.open("CSI_0.txt", std::ios::app);
	}else{
		if(d_channel==1){
			myfile1.open("CSI_1.txt", std::ios::app);
		}else{
			myfile1.open("CSI_other.txt", std::ios::app);
		}
	}

	while(i < ninput_items[0]) {
		get_tags_in_range(all_tags, 0, nread + i, nread + i + 1);
		...
		if(tags.size()) {
			...
			// [C.S.] 			
			// dout<<"Start decoding MAC address "<< std::endl;
			// dout<<"Extract CSI "<< std::endl;
			// myfile1<<"len_data: "<< len_data << std::endl;

			get_tags_in_range(tags_csi, 0, nread + i, nread + i + 1, pmt::string_to_symbol("preamble"));
			pmt::pmt_t dict_csi = tags_csi[0].value; 

			d_sof = pmt::to_uint64(pmt::dict_ref(dict, pmt::mp("sof"), pmt::from_uint64(0)));
			// d_channel = pmt::to_long(pmt::dict_ref(dict, pmt::mp("channel"), pmt::from_long(10)));
			frame_start = pmt::to_uint64(pmt::dict_ref(dict, pmt::mp("frame_start"), pmt::from_uint64(0)));
			...
			myfile1 << d_sof << std::endl;
			myfile1 << frame_start << std::endl;
			pmt::pmt_t preamble_in_f = pmt::make_c32vector(64, 0); 
			preamble_in_f = pmt::dict_ref(dict_csi, pmt::mp("pre_in_f"), pmt::from_complex(0));
			// dout <<"tags_csi:"<< tags_csi << std::endl;

			// myfile1 << "start" << std::endl;
			// myfile1 << d_channel << std::endl;
			// myfile1 << "end" << std::endl;

			// add timestamp
			gettimeofday(&tv, NULL);
			long now = tv.tv_sec * 1000000 + tv.tv_usec;
			myfile1 << now << std::endl;


			myfile1 << preamble_in_f << std::endl;
			...
			// if(frame.n_sym <= 2000) {
				...
				d_frame = frame;
				copied = 0;
                                // dout << "frame length:" << frame.n_sym <<std::endl;
...
void decode() {
	...
	// dict = pmt::dict_add(dict, pmt::mp("channel"), pmt::from_long(d_channel));
...
private:
	...
	frame_param d_frame;
	ofdm_param d_ofdm;
	uint64_t d_sof;  // start-of-frame [X.Z. 2019-03-26]
...
decode_mac::sptr
decode_mac::make(bool log, bool debug, int channel) {
	return gnuradio::get_initial_sptr(new decode_mac_impl(log, debug, channel));
}
